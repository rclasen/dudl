...
\begin{description}
\item[User]
	Wer hat welchen login und Passwort. Welche Rechte gehören zu einem
	login.
\item[Musik] 
	Welche Titel gehören zu welchem Album eines Künstlers.
\item[Rating] 
	In welches Genre sortiert ein User einen Titel. Wie gerne hört er
...
\item[Player] 
	Auswählen und abspielen von tracks.
\item[Video] 
	Was ist auf Video Kassetten.
\item[Verleih] 
	Wer hat welches Video oder Musik Träger wann ausgeliehen.
\end{description}

Die Datenbank soll mindestens über ein Web Interface abfrag- und
editierbar sein. 

\subsection{Player}

Der Player soll Modular aufgebaut sein. Die einzelnen Teile müssen direkt
...
zur verfügung. Zu Begin ist aber denkbar, daß die anderen Komponenten noch
...
werden um ereignisgesteuert vorgehen zu können.

Der Player wird in folgende Einheiten zerteilt:

\begin{description}
\item[Frontend]
	Ein Frontend ist für das User interface zuständig. Es kann 
	über ein eigenes Protokoll mit dem Backend kommunizieren. 

\item[Player]
	Der eigentlich Player ist ein Teil des Backends. Er kümmert sich
	dadrum, den richtigen Track zu spielen. Er reagiert auf die
	frontends und nimmt sich die nötigen Daten aus der Datenbank. Der
	Player bereitet Die Daten für den Playback Client auf. Nur ein
	einziger Playback Client kann mit einem Player verbunden sein. Es
	sollen mehrere Player laufen können.

\item[Playback Client]
	Verbindet sich zu einem Player des Backends. Er nimmt die Daten
	in allen für ihn verwertbaren formaten entgegen und sorgt dafür,
	daß sie abgespielt werden können - zB. auf /dev/dsp, über ESD oder
	über einen schoutcast server.

	Playback client und Player handeln zu begin aus, welche Formate
	vom client unterstützt werden.

	Später sollen auch die Musikdaten vom Player zum Playback Client
...
\item[Backend] 
	...
	verbinden sich mit dem Backend und erhalten über das Backend
...
\item aufnahmequalität
\item wann gekauft
\item meine eigene CD - sprich in meinem Schrank, oder nur mal geliehen
gewesen und für CDDB/kopieren eingelesen?
\item für Hüllen (Kassette) Inlays erzeugen
...
\item erweiterbar für neue Medien
...
eines albums, das dann automatisch übernommen wird.
...
\item Zu welchen Gruppen gehört er?
\end{itemize}
...
\section{Rating}
\begin{itemize}
\item zu welchem genre gehört ein track?
\item wie gefällt mir der track
\item wie gefällt mir ein genre?
\item zusammenstellen von programmen aus storage
\item Programme aufbrechen in happen bestimmter Spielzeit
\item Programm für Medium speichern
\end{itemize}
...
\item queue aus programmen füllen
\item Trackweise in queue vor- und in history rückwärts springen
\item Random play gemäß filter und rating der lauschenden user/History
\end{itemize}

\subsection{Zusammenspiel queue und History}

Diese Sprünge in der queue\ldots
\\
